...
Then il est redirigé vers un formulaire affichant une liste vide d'ordres de livraison
    [Documentation]
    Wait Until Page Contains   Ordres de livraison - Sté PRIOS - Etablissement CARQUEFOU
    Capture Page Screenshot

Given l'utilisateur est sur la liste des "Ordres de livraison"

    [Documentation]
    Wait Until Page Contains    Ordre(s) de livraison    10s
    Capture Page Screenshot

When il fait un clic droit sur la liste
    Wait Until Element Is Visible    xpath=//table    5s
    Open Context Menu    xpath=//table
    Sleep    2s
